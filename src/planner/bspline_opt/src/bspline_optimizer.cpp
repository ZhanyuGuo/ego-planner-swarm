...
void BsplineOptimizer::setSwarmTrajs(SwarmTrajData* swarm_trajs_ptr)
{
...
std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
{
  ...
  for (int i = 0; i < seg_upbound; i++)
  {
    ...
    //     {
    //       cout << "      " << RichInfoOneSeg_temp.base_point[k][k2].transpose() << " @ " <<
    //       RichInfoOneSeg_temp.direction[k][k2].transpose() << endl;
    //     }
    //   }
  }
  ...
  for (int i = 0; i < seg_upbound; i++)
  {
    ...
    if (RichInfoSegs[i].first.size > 1)
    {
      ...
      for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++)
      {
        // cout << "A *" << j << "*" << endl;
        double step_size =
            RESOLUTION / (RichInfoSegs[i].first.points.col(j) - RichInfoSegs[i].first.points.col(j + 1)).norm() / 2;
    ...
    exit_multi_loop1:;
      for (int j = RichInfoSegs[i].first.size - 1; j >= 1; j--)
      {
        ...
        ;
        double step_size =
            RESOLUTION / (RichInfoSegs[i].first.points.col(j) - RichInfoSegs[i].first.points.col(j - 1)).norm();
        for (double a = 1; a > 0; a -= step_size)
        {
          Eigen::Vector3d pt(a * RichInfoSegs[i].first.points.col(j) +
                             (1 - a) * RichInfoSegs[i].first.points.col(j - 1));
    ...
    exit_multi_loop2:;
      ...
      if (occ_start_id == -1 || occ_end_id == -1)
      {
        ...
        //     {
        //       cout << "      " << RichInfoSegs[i].first.base_point[k][k2].transpose() << " @ " <<
        //       RichInfoSegs[i].first.direction[k][k2].transpose() << endl;
        //     }
        //   }
        // }
      }
      ...
      for (int j = occ_start_id; j <= occ_end_id; j++)
      {
        ...
        if (RichInfoSegs[i].first.base_point[j].size() != 1)
        {
          cout << "RichInfoSegs[" << i << "].first.base_point[" << j
               << "].size()=" << RichInfoSegs[i].first.base_point[j].size() << endl;
        ...
        else
        {
          base_pt_reverse =
              ...
              base_vec_reverse * (RichInfoSegs[i].first.base_point[j][0] - RichInfoSegs[i].first.points.col(j)).norm();
        }
        ...
        else if ((base_pt_reverse - RichInfoSegs[i].first.points.col(j)).norm() >= RESOLUTION)  // Unnecessary to
                                                                                                // search.
        {
      ...
      // 1.3 Assign the base points to control points within [0, occ_start_id) and (occ_end_id,
      // RichInfoSegs[i].first.size()-1].
    ...
    else if (RichInfoSegs[i].first.size == 1)
    {
      ...
      Eigen::Vector3d base_pt_reverse =
          ...
          base_vec_reverse * (RichInfoSegs[i].first.base_point[0][0] - RichInfoSegs[i].first.points.col(0)).norm();
      ...
      else
      {
        ...
        cout << "base_point=" << RichInfoSegs[i].first.base_point[0][0].transpose()
             << " control point=" << RichInfoSegs[i].first.points.col(0).transpose() << endl;
  ...
  for (int i = 0; i < max_traj_nums; i++)
  {
    ...
    while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.size)
    {
      ...
      //  {
      //    cout << "cp_of_seg_id=" << cp_of_seg_id << " RichInfoSegs[seg_id].first.base_point.size()=" <<
      //    RichInfoSegs[seg_id].first.base_point.size() << endl;
      //  }
      ...
      {
        ROS_ERROR(
            "Shold not happen!!!!, cp_id=%d, seg_id=%d, segments.front().first=%d, segments.back().second=%d, "
            "segments[seg_id].first=%d, segments[seg_id].second=%d",
...
std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd& init_points,
                                                                     bool flag_first_init /*= true*/)
{
  ...
  int i_end =
      (int)init_points.cols() - order_ - ((int)init_points.cols() - 2 * order_) / 3;  // only check closed 2/3 points.
  ...
  for (size_t i = 0; i < segment_ids.size(); i++)
  {
    ...
    else
    {
      id_low_bound =
          (int)(((segment_ids[i].first + segment_ids[i - 1].second) + 1.0f) / 2);  // id_low_bound : +1.0f ceil()
      id_up_bound =
          (int)(((segment_ids[i].second + segment_ids[i + 1].first) - 1.0f) / 2);  // id_up_bound : -1.0f fix()
    }
  ...
  constexpr double MINIMUM_PERCENT =
      0.0;  // Each segment is guaranteed to have sufficient points to generate sufficient force
  ...
  for (size_t i = 0; i < segment_ids.size(); i++)
  {
    ...
    // cout << "final:" << "i = " << i << " first = " << adjusted_segment_ids[i].first << " second = " <<
    // adjusted_segment_ids[i].second << endl;
  }
  ...
  for (size_t i = 0; i < segment_ids.size(); i++)
  {
    ...
    for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
    {
      ...
      if (got_intersection_id >= 0)
      {
        ...
        if (length > 1e-5)
        {
          ...
          {
            occ = grid_map_->getInflateOccupancy((a / length) * intersection_point +
    ...
    {
      Eigen::Vector3d ctrl_pts_law(init_points.col(segment_ids[i].second) - init_points.col(segment_ids[i].first)),
          intersection_point;
  ...
  for (int i = order_; i < end_idx; i++)
  {
    ...
    for (size_t id = 0; id < swarm_trajs_->size(); id++)
    {
      ...
      if (glb_time < traj_i_satrt_time + swarm_trajs_->at(id).duration_ - 0.1)
      {
        ...
        double ellip_dist =
            sqrt(dist_vec(2) * dist_vec(2) * inv_a2 + (dist_vec(0) * dist_vec(0) + dist_vec(1) * dist_vec(1)) * inv_b2);
...
void BsplineOptimizer::calcMovingObjCost(const Eigen::MatrixXd& q, double& cost, Eigen::MatrixXd& gradient)
{
  ...
  for (int i = order_; i < end_idx; i++)
  {
    ...
    for (int id = 0; id < moving_objs_->getObjNums(); id++)
    {
      ...
      // cout /*<< "cps_.points.col(i)=" << cps_.points.col(i).transpose()*/ << " moving_objs_=" << obj_prid.transpose()
      // << " dist=" << dist << endl;
                                               ...
                                               int iter_num, double smoothness_cost)
{
  ...
  if (iter_num > 3 && smoothness_cost / (cps_.size - 2 * order_) < 0.1)  // 0.1 is an experimental value that indicates
                                                                         // the trajectory is smooth enough.
  {
...
bool BsplineOptimizer::rebound_optimize(double& final_cost)
{
  ...
  do
  {
        ...
        result == lbfgs::LBFGS_ALREADY_MINIMIZED || result == lbfgs::LBFGS_STOP)
    {
      ...
      // cout << "XXXXXX" << ((cps_.points.col(cps_.points.cols()-1) + 4*cps_.points.col(cps_.points.cols()-2) +
      // cps_.points.col(cps_.points.cols()-3))/6 - local_target_pt_).norm() << endl;
...
bool BsplineOptimizer::refine_optimize()
{
  ...
  do
  {
    ...
    int result = lbfgs::lbfgs_optimize(variable_num_, q, &final_cost, BsplineOptimizer::costFunctionRefine, NULL, NULL,
                                       this, &lbfgs_params);
    if (result == lbfgs::LBFGS_CONVERGENCE || result == lbfgs::LBFGSERR_MAXIMUMITERATION ||
        result == lbfgs::LBFGS_ALREADY_MINIMIZED || result == lbfgs::LBFGS_STOP)
    {
    ...
    double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() /
                                  grid_map_->getResolution());  // Step size is defined as the maximum size that can
                                                                // passes throgth every gird.
...
void BsplineOptimizer::combineCostRebound(const double* x, double* grad, double& f_combine, const int n)
{
  ...
  // f_mov_objs; printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);
