...
void EGOReplanFSM::init(ros::NodeHandle& nh)
{
  ...
  broadcast_bspline_sub_ =
      nh.subscribe("planning/broadcast_bspline_to_planner", 100, &EGOReplanFSM::BroadcastBsplineCallback, this,
                   ros::TransportHints().tcpNoDelay());
...
void EGOReplanFSM::BroadcastBsplineCallback(const traj_utils::BsplinePtr& msg)
{
  ...
  {
    ROS_ERROR("Time difference is too large! Local - Remote Agent %d = %fs", msg->drone_id,
  ...
  else
  {
    ...
    planner_manager_->swarm_trajs_buf_[id].duration_ =
        (msg->knots[msg->knots.size() - floor(cutback)] + msg->knots[msg->knots.size() - ceil(cutback)]) / 2;
  }
...
void EGOReplanFSM::swarmTrajsCallback(const traj_utils::MultiBsplinesPtr& msg)
{
  ...
  // cout << "\033[45;33mmulti_bspline_msgs_buf.drone_id_from=" << multi_bspline_msgs_buf_.drone_id_from << "
  // multi_bspline_msgs_buf_.traj.size()=" << multi_bspline_msgs_buf_.traj.size() << "\033[0m" << endl;
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent& e)
{
  ...
  {
    if (t_cur < t_2_3 && t >= t_2_3)  // If t_cur < t_2_3, only the first 2/3 partition of the trajectory is considered
