                                      ...
                                      bool flag_polyInit, bool flag_randomPolyTraj)
{
  ...
  // local_target_pt.transpose() << ", " << local_target_vel.transpose()
  //      << endl;
    ...
    else  // Initial path generated from previous trajectory.
    {
      ...
      do
      {
        ...
        while ((id <= pseudo_arc_length.size() - 2) && sample_length <= pseudo_arc_length.back())
        {
          ...
          {
            point_set.push_back((sample_length - pseudo_arc_length[id]) /
  ...
  cout << "total time:\033[42m" << (t_init + t_opt + t_refine).toSec()
       << "\033[0m,optimize:" << (t_init + t_opt).toSec() << ",refine:" << t_refine.toSec()
...
bool EGOPlannerManager::checkCollision(int drone_id)
{
  ...
  for (double t = t_start; t < t_end; t += 0.03)
  {
    if ((local_data_.position_traj_.evaluateDeBoorT(t - my_traj_start_time) -
         swarm_trajs_buf_[drone_id].position_traj_.evaluateDeBoorT(t - other_traj_start_time))
